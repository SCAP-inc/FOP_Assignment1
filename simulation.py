...
import cv2
import time
from animals import Duck, Newt, Shrimp
...
def main():
    ...
    for d in ducks:
            allCreatures.append(d)
    for n in ducks:
            allCreatures.append(n)
    for s in shrimps:
            allCreatures.append(s)
    ...
    for i in range(TIMESTEPS):
        ...
        for c in allCreatures:
            c.stepChange(terrain, heightmap, allCreatures)
            if c.name == "duck":
                if c.state == "egg":
                    d.stepChange(terrain, heightmap, ducks)
                    DuckEggXValues.append(c.pos[0])
                    DuckEggYValues.append(c.pos[1])
                    DuckEggSizes.append(c.getSize() * 25)
                if c.state == "adult":
                    d.stepChange(terrain, heightmap, ducks)
                    DuckAdultXValues.append(c.pos[0])
                    DuckAdultYValues.append(c.pos[1])
                    DuckAdultSizes.append(c.getSize() * 50)
                    if c.age > 30:
        ...
        plt.scatter(DuckAdultXValues, DuckAdultYValues, s=DuckAdultSizes, color="blue", marker=duck,label="Duck")  # Note plt origin is bottom left

        ax.imshow(terrain)
        # plt.scatter(xvalues, yvalues, s=sizes, color="orange")  # Note plt origin is bottom left
        plt.xlim(0, XMAX)
        plt.ylim(0, YMAX)
        plt.axis("off")
        plt.pause(1)
